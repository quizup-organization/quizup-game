...
public class GameProjection {
        ...
        gameRepositoryPort.findById(event.gameId()).ifPresent(game ->
                gameRepositoryPort.save(game.toBuilder()
                        .rounds(updateRound(game.rounds(), event.round(), round -> round.toBuilder()
                                .status(GameRoundStatus.QUESTION_SHOWN)
                                .build()))
                        .build())
        );
    }
    ...
    public void on(GameEvent.QuestionRevealedEvent event) {
        logger.debug("Projecting QuestionRevealedEvent: gameId={}, round={}", event.gameId(), event.round());
        gameRepositoryPort.findById(event.gameId()).ifPresent(game ->
                gameRepositoryPort.save(game.toBuilder()
                        .rounds(updateRound(game.rounds(), event.round(), round -> round.toBuilder()
                                .status(GameRoundStatus.ANSWERABLE)
                                .revealedAt(event.revealedAt())
                                .answerDeadlineAt(event.answerDeadlineAt())
                                .build()))
                        .build())
        );
    }
